...
class Client:
    def __init__(self, username=None, password=None, token=None,
        ...
        if self.token:
            self.token_exp = jwt.decode(jwt=self.token, verify=False,
                                        options=DECODE_OPTIONS)['exp']
        else:
            self.token_exp = 0.
...
class AdminClient(Client):
    ...
    def upload_file_distribution(self, distribution_filepath):
        ...
        return requests.post(url.format(self.api_url), files=files,
                             headers=self.auth_header, proxies=self.proxies)
    ...
    def create_git_distribution(self, http_url, username, token):
        ...
        return requests.post(url.format(self.api_url), json=data,
                             headers=self.auth_header, proxies=self.proxies)
    ...
    def update_user(self, user_id: int, first_name: str = None,
                    ...
                    admin: bool = None):
        # TODO. This method looks weird. See following TODOs
        data = {}
        for attr_name, attr_value in [('first_name', first_name),
                                      ('last_name', last_name),
                                      ('active', active),
                                      ('admin', admin)]:
            # TODO Why is this list not a dict ?
            if attr_value:
                data['attr_name'] = attr_value
                # TODO This looks wrong. We only put 'attr_name' in data. Not the actual attribute
